...
describe('Investors', () => {
    ...
    it('updateInvestor', async () => {
        ...
        await expect(investors.getInvestorIdByWallet(testWallet1, sender)).resolves.toBe(
            testInvestor1
        )
        await expect(investors.getCountry(testInvestor1, sender)).resolves.toBe(Country.US)
        await expect(
            investors.getAttributeValue(testInvestor1, AttributeType.KYC_APPROVED, sender)
        ).resolves.toBe(AttributeStatus.APPROVED)
        await expect(
            investors.getAttributeExpiration(testInvestor1, AttributeType.KYC_APPROVED, sender)
        ).resolves.toBe(111n)
        await expect(
            investors.getAttributeValue(testInvestor1, AttributeType.PROFESSIONAL, sender)
        ).resolves.toBe(AttributeStatus.PENDING)
        await expect(
            investors.getAttributeExpiration(testInvestor1, AttributeType.PROFESSIONAL, sender)
        ).resolves.toBe(222n)
    ...
    it('update attributes', async () => {
        ...
        await expect(
            investors.getAttributeValue(testInvestor1, AttributeType.ACCREDITED, sender)
        ).resolves.toBe(AttributeStatus.REJECTED)
        await expect(
            investors.getAttributeExpiration(testInvestor1, AttributeType.ACCREDITED, sender)
        ).resolves.toBe(333n)
