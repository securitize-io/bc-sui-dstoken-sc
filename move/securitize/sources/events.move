...
public(package) fun emit_authorized_securities_rule_created_event<T>(max_supply: u64) {
    event::emit(DSComplianceAuthorizedSecuritiesRuleCreated<T> { max_supply });
}

public(package) fun emit_backdating_issuance_rule_created_event<T>(disallow_backdating: bool) {
